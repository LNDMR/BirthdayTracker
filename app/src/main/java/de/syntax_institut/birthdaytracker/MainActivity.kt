...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val calculate_button = findViewById<Button>(R.id.calculate_button)
    ...
    private fun getValuesFromInput() {
        ...
        val nameInput = findViewById<TextView>(R.id.name_text).text
        val surnameInput = findViewById<TextView>(R.id.surname_text).text

        val daySpinner = findViewById<Spinner>(R.id.days_spinner).selectedItem.toString().toInt()
        val monthSpinner = findViewById<Spinner>(R.id.months_spinner).selectedItem.toString()
        ...
        var salutation = findViewById<TextView>(R.id.salutation_text)
        var message = findViewById<TextView>(R.id.result_message_text)

        salutation.text = getString(R.string.salutation,nameInput,surnameInput)

        if(daysToBirthday == 0){
            message.text = "It´s your birthday today!!!!"
        } else {
            message.text = getString(R.string.days_until_birthday_message,daysToBirthday)
        }
